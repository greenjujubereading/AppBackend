...
public class HomePageController {
    ...
    public List<BookList> getBookListByYear(String video_year) {
        ...
        for (int i = 0; i < videoYearArray.length; i++) {
            System.out.println(videoYearArray[i]);
            booklist.addAll(homePageService.getBookListByYear(videoYearArray[i]));
        }
...
//		System.out.println(video_id);
    ...
    public Map<String, Object> collecte(UserCollection uc) {
        ...
        if (usercollection == null) {
            ...
            homePageService.deleteCollect(uc);
    ...
    public List<BookList> getcollection(String user_id) {
        List<UserCollection> userCollections = homePageService.getcollection(user_id);
        List<BookList> booklists = new ArrayList<BookList>();
        for (int i = 0; i < userCollections.size(); i++) {
            Video video = homePageService.getBookById(userCollections.get(i).getVideo_id().toString());
            BookList booklist = new BookList();
            booklist.setVideo_id(video.getVideo_id());
            booklist.setName(video.getName());
            booklist.setVideo_cover_url(FilePath.getUrl(video.getVideo_cover_url()));
    ...
    public List<BookList> gethistory(String user_id) {
        List<UserHistory> userHistoryList = homePageService.gethistory(user_id);
        List<BookList> booklists = new ArrayList<BookList>();
        for (int i = 0; i < userHistoryList.size(); i++) {
            Video video = homePageService.getBookById(userHistoryList.get(i).getVideo_id().toString());
            BookList booklist = new BookList();
            booklist.setVideo_id(video.getVideo_id());
            booklist.setName(video.getName());
            booklist.setVideo_cover_url(FilePath.getUrl(video.getVideo_cover_url()));
...
//    }
    ...
    @CrossOrigin
    @ResponseBody
    public Map<String, Object> commentToVideo(CommentCustom comment) {
        System.out.println(comment);
        System.out.println("video id: " + comment.getVideo_id());
        System.out.println("from user id: " + comment.getFrom_user_id());
        System.out.println("cotent: " + comment.getCotent());
        comment.setComment_date( new Date());
        boolean result = true;
        try {
            homePageService.addANewComment(comment);
        } catch (Exception e) {
            result = false;
        }
        Map map = new HashMap<String, Object>();
